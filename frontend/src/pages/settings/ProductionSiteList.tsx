...
export default function ProductionSiteList() {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/production-sites', { name: newName });
            setNewName('');
            fetchSites();
        } catch (e) {
    ...
    const saveEditing = async () => {
        ...
        try {
            await api.put(`/production-sites/${editingId}`, { name: editingName });
            setEditingId(null);
            fetchSites();
        } catch (e) {
        ...
        <div className="p-6">
            ...
            <form onSubmit={handleAdd} className="flex gap-2 mb-6 max-w-md">
                <input
                    ...
                    placeholder="Nouveau lieu (ex: Usine Nord)"
                    value={newName}
            ...
            <ul className="bg-white shadow rounded-md max-w-md divide-y">
                ...
                {sites.length === 0 && (
                    <li className="p-4 text-center text-gray-500 italic">Aucun lieu de production défini.</li>
                )}
            </ul>
        </div>
    );
