...
import PartCategoryEditModal from './PartCategoryEditModal';
...
const PartCategoryList: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm('Êtes-vous sûr de vouloir supprimer cette catégorie ?')) {
            try {
                const res = await fetch(`/api/part-categories/${id}`, { method: 'DELETE' });
                if (res.ok) {
    ...
    const handleSave = async (updatedData: Partial<PartCategory>) => {
        try {
            const url = selectedCategory ? `/api/part-categories/${selectedCategory.id}` : '/api/part-categories';
            ...
            console.error('Error updating/creating part category:', error);
            alert('Erreur lors de la modification/création');
        }
    };
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl font-bold mb-6">Catégories de Pièces</h1>
