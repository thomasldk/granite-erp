...
// --- CONFIGURATION ---
const AGENT_VERSION = "V5.10 - ID INJECTION FIX";
...
async function processJob(filename) {
    ...
    // If not found (legacy RAK), fallback to filename split but it might be Reference now
    // If quoteIdMatch is missing, and filename is Ref, download will fail unless we are in generate mode which doesn't need downloading source?
    // Actually Generate mode needs downloading Result.
    ...
    // UNIVERSAL DOWNLOAD (V5.9 Logic): Download source for ANY action if we have a target path & ID
    if (targetPath && quoteId) {
        console.log(`🔄 ACTION DETECTED (Generate/Reint) -> Target: ${targetPath}`);

        // We try to download the Excel for this quote using the UUID
        ...
        try {
            ...
            // If download fails here, we continue because maybe the file is already there?
            // User complained about "Exists=false", so this success is crucial.
        }
    }
...
<ligne No="1" Ref="Simulated V5.10" Description="Success ID Injection" QTY="1" Prix="100.00" Total="100.00" />
