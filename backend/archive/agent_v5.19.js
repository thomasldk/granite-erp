/* AGENT V5.19 - PDF NO-COPY (FIX CONFLICT) */
...
// --- CONFIGURATION ---
const AGENT_VERSION = "V5.19 - PDF NO-COPY";
    ...
    if (action === 'devispdf' && sourcePath) {
        ...
        try {
            ...
            if (fs.existsSync(sourcePath)) {
                ...
                // --- CLEANUP: DELETE EXISTING FILE TO AVOID "REPLACE?" PROMPT ---
                if (fs.existsSync(destExcelPath)) {
                    try {
                        fs.unlinkSync(destExcelPath);
                        console.log(`   🧹 Cleanup: Deleted old file.`);
                        // Wait for FS to release lock/sync
                ...
                // --- ENABLE COPY FOR PDF (User Requirement) ---
                fs.copyFileSync(sourcePath, destExcelPath);
                console.log(`   ✅ Pre-Copy Success: Excel copied to ${destExcelPath}`);
...
async function waitForReturn(originalRakName, excelPath, jobStartTime, action = '', dirPdf = '') {
    ...
    if (foundFile) {
        ...
        // HANDLE PDF UPLOAD
        if (action === 'devispdf') {
            // Logic: The Excel Filename was "DRC25-XXX.xlsx"
            // The PDF should be "DRC25-XXX.pdf" in dirPdf
            const baseName = originalRakName.replace(/\.(rak|xml)$/i, '').replace('.xlsx', ''); // Safety clean
            // Actually, originalRakName includes extension usually? 
            // The filename passed to processJob is usually "File.xlsx.rak" or just "File.rak" depending on backend.
            // Let's assume the PDF has the same basename as the XML return but with .pdf

            // Better: use the basename of the XML return file (without .xml) + .pdf
            const pdfFilename = foundFile.replace(/\.xml$/i, '.pdf');
            const cleanDirPdf = dirPdf ? dirPdf.replace(/\\$/, '') : 'F:\\nxerppdf';
            const pdfPath = path.join(cleanDirPdf, pdfFilename);
