    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (name) {
            ...
            if (existing) {
                ...
                currentPart = await prisma.part.create({
                    data: {
                        name,
                        reference,
                        description,
            ...
            if (currentPart) {
                ...
                if (cols[9]?.trim()) updateData.description = (currentPart.description ? currentPart.description + '\n' : '') + cols[9].trim();
                if (cols[10]?.trim()) updateData.note = (currentPart.note ? currentPart.note + '\n' : '') + cols[10].trim();
                if (cols[11]?.trim()) updateData.supplier = (currentPart.supplier ? currentPart.supplier + '\n' : '') + cols[11].trim();

                if (Object.keys(updateData).length > 0) {
                    currentPart = await prisma.part.update({
