...
// --- CONFIGURATION ---
const AGENT_VERSION = "V22 - NO PDF (FIXED)";
...
async function processJob(filename) {
    ...
    // --- ACTION HANDLERS (Mutually Exclusive) ---
    if (action === 'devispdf') {
        console.log(`📄 PDF GENERATION DETECTED (RESET MODE V21)`);
        if (sourcePath) console.log(`   Source: ${sourcePath}`);
        console.log(`   ℹ️ No Pre-Action (Copy/Clean) configured yet. Waiting for Step 1.`);

    } else if (action === 'recopier' || action === 'reviser') {
